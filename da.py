    ...
    for tab_cv in range(cross_cv):
        ...
        if Label == "MS-LSTM":
            ...
            if is_multi_scale == True and each_case == 2:
                ...
                print(tf.transpose(u_w_scales_normalized).get_shape())
            ...
            elif is_multi_scale == True and each_case == 1:
                number_scale_levels = training_level
                u_w_scales_normalized = tf.Variable(tf.constant(1.0/number_scale_levels,shape=[1,number_scale_levels]), name="u_w")
                u_w_scales_normalized = normalized_scale_levels(u_w_scales_normalized)
                u_w = tf.Variable(tf.random_normal(shape=[1,sequence_window]), name="u_w")
    ...
    try:
        ...
        if is_add_noise == False:
            if corss_val_label == 0:
                outputfilename = "TabCAV_A_MS-LSTM_Log_"+filename+".txt"

            else:
                outputfilename = "TabCAV_B_MS-LSTM_Log_"+filename+".txt"
            with open(os.path.join(os.getcwd(),outputfilename),"a")as fout:
                time_str = time.strftime('%Y-%m-%d-%H:%M:%S', time.localtime())
                time_str = "Time:"+ time_str + ","
                if training_level>0:
                    outfileline = time_str+Label+"_epoch:"+str(epoch_stop)+",__wavelet type:"+str(wave_type)+",__pooling type:"+str(pooling_type)+",__learning rate:"+str(learning_rate)+",__multi_scale:"+str(is_multi_scale)+",__scale_levels:"+str(training_level)+",__sequence_window:"+str(sequence_window)+"\n"
                else:
                    outfileline = time_str+Label+"_epoch:"+str(epoch_stop)+",__wavelet type:"+str(wave_type)+",__learning rate:"+str(learning_rate)+",__multi_scale:"+str(is_multi_scale)+",__scale_levels:"+str(training_level)+",__sequence_window:"+str(sequence_window)+"\n"
            ...
            with open(os.path.join(os.getcwd(), "MS-LSTM_Log_Adding_Noise_" + filename + ".txt"), "a")as fout:
                if training_level > 0:
                    outfileline = time_str + Label + "_____epoch:" + str(epoch_stop) +",_____pooling type:"+str(pooling_type)+ ",_____learning rate:" + \
        ...
        for tab in range(len(corss_val_label_list)):
            ...
            for sequence_window in sequence_window_list:
                ...
                for each_case in case_list:
                    if each_case == 0:
                        is_multi_scale = False#Original Single-Scale LSTM
                        training_level = -1

                    elif each_case == 1:# MS-LSTM
                        is_multi_scale = True

                        training_level = multi_scale_value

                    elif each_case == 2:# MS-LSTM + Attention LSTM
                        is_multi_scale = True
                        training_level = multi_scale_value
                    elif each_case == 3:# Single-Scale LSTM
                        is_multi_scale = False
                        for t in range(1,multi_scale_value+1):
                            training_level = t
