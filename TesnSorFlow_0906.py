...
def Model(Label):
    ...
    for tab_cv in range(cross_cv):
        ...
        if Label == "Keras-LSTM":
            ...
            number_of_scales = len(x_train_multi_list[0])
            u_w = tf.Variable(tf.random_normal(shape=[number_of_scales]), name="u_w")
            ...
            current_level = tf.placeholder(tf.int32,shape=[1])
            target = tf.placeholder(tf.float32, [None, number_class])


            data_original_train_current_level = data_original_train

            lstm_cell = tf.nn.rnn_cell.BasicLSTMCell(num_neurons, forget_bias=1.0, activation=tf.nn.tanh)

            val, state = tf.nn.dynamic_rnn(lstm_cell, data_original_train_current_level, dtype=tf.float32)
            val = tf.transpose(val, [1, 0, 2])

            m_current_level = tf.gather(val, int(val.get_shape()[0]) - 1)

            print(m_current_level.get_shape())
            print(m_current_level)

            #m_total = tf.pad([],m_current_level)
            #if current_level > 0:
            #    m_total = tf.pad(m_total,m_current_level)
            #m_total = m_total.reshape(number_of_scales,num_neurons)

            Weight_W = tf.Variable(tf.truncated_normal([num_neurons,number_of_scales]))
            b_W = tf.Variable(tf.constant(0.1, shape=[number_of_scales]))
            u_current_level = tf.tanh(tf.matmul(m_current_level, Weight_W) + b_W)

            w_current_level_temp = tf.exp(tf.mul(tf.transpose(u_w),u_current_level))

            print(tf.gather(w_current_level_temp,1).get_shape())
            w_total_temp = tf.pad([],tf.gather(w_current_level_temp,1))
            if current_level > 0:
                w_total_temp = tf.pad(w_total_temp,w_current_level_temp)
            w_current_level = tf.div(w_current_level_temp,tf.sum(w_total_temp))



            w_total = tf.pad([],[w_current_level])
            if current_level > 0:
                w_total = tf.pad(w_total,w_current_level)



            m_ = tf.mul(w_total,m_total)
            prediction = tf.nn.softmax(m_)
            ...
            for i in range(epoch):
                ...
                for j in range(no_of_batches):
                    ...
                    ptr += batch_size
                    for k in range(multi_scale_value):
                        print('input shape')
                        print(inp.shape)
                        sess.run(minimize, {data_original_train_current_level: inp, current_level:k,target: out})
