...
def Model(Label):
    ...
    for tab_cv in range(cross_cv):
        ...
        x_train_multi_list,x_train, y_train, x_testing_multi_list,x_test, y_test = LoadData.GetData(filepath, filename, sequence_window,tab_cv,cross_cv,Multi_Scale=True,Wave_Let_Scale=multi_scale_value)
        ...
        if is_multi_scale:
            ...
            if training_level < 0:
                ...
                #x_test = x_testing_multi_list[training_level]
            pass
        ...
        if Label == "Keras":
            ...
            output_scale_weight = tf.Print(scale_weight, [scale_weight], message="This is Scale_Weight: ")

            data_original_train = tf.placeholder(tf.float32, [number_of_scales,batch_size,sequence_window,input_dim])
            data_for_lstm_0 = tf.Variable(tf.zeros((batch_size,sequence_window,input_dim)),name="input_for_lstm")
            ...
            for each_scale in range(number_of_scales):
                ...
                data_for_lstm_0 += tf.mul(scale_weight[each_scale], tf.gather(data_original_train, each_scale))
                #c = tf.Print(data_for_lstm_0, [data_for_lstm_0], message="This is Input For LSTM Shape: ")


            if is_multi_scale:
                data_for_lstm_multi_scale = data_for_lstm_0
                """
                ...
                if training_level == 0:
                    pass
                    """
                    out_put_before_multi_first_level = tf.Print(data_original_train,[tf.gather(tf.gather(data_original_train, 150), 0)],\
                                                message="this is output for first level but we do not multiply weight................................ ",\
                                                first_n=1024, summarize=10)
                    """
