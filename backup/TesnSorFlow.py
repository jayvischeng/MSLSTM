...
def RNN(_X, _istate, _weights, _biases):
    ...
    lstm_cell = rnn_cell.LSTMCell(n_hidden,(n_steps,n_input))
                ...
                for tab_cross in range(cross_folder):
                    ...
                    if window_size_label == "true":
                        if methodLabel == 0:
                            ...
                            #    Temp.append([each])
                            #Y_Training = np.array(Temp)
                            #Temp = []
                            #for each in Y_Testing:
                            #    Temp.append([each])
                            #Y_Testing = np.array(Temp)
                            ...
                            global n_input,n_steps,n_hidden,n_classes
                            n_input = 33
                            n_steps = len(X_Training[0])  # timesteps
                            ...
                            cost = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(pred, y, name=None))#Sigmoid
                            ...
                            with tf.Session() as sess:
                                ...
                                while total_step * batch_size < training_iters:


                                    batch_xs, batch_ys = (X_Training[(step-1)*batch_size:step*batch_size],Y_Training[(step-1)*batch_size:step*batch_size])
                                    ...
                                    #batch_ys.reshape(len(batch_ys),1)
                                    ...
                                    if step % display_step == 0:
                                        ...
                                        print "Iter " + str(total_step * batch_size) + ", Minibatch Loss= " + "{:.6f}".format(
                                            loss) + \
                                ...
                                print "Testing Accuracy:", sess.run(accuracy, feed_dict={x: test_data, y: test_label,istate: np.zeros((len(Y_Testing), 2 * n_hidden))})
                                #result = sess.graph
