...
def Model(Label):
    ...
    for tab_cv in range(cross_cv):
        ...
        if Label == "Keras-LSTM":
            ...
            #val shape is (batch_size,time_stamp,hidden units)
            ...
            b_W = tf.Variable(tf.constant(0.1, shape=[number_of_scales,number_of_scales]))
            ...
            u_current_levels_temp = tf.matmul(val,Weight_W)+b_W

            u_current_levels_total = tf.cumsum(tf.exp(tf.mul(u_current_levels_temp,tf.transpose(u_w))))
            u_current_levels = tf.div(tf.exp(tf.mul(u_current_levels_temp,tf.transpose(u_w))),u_current_levels_total)
            target = tf.placeholder(tf.float32, [batch_size, number_class])
            print(tf.mul(u_current_levels_temp,tf.transpose(u_w)).get_shape())
            m_total = tf.matmul(u_current_levels,val)

            prediction = tf.nn.softmax(m_total)
